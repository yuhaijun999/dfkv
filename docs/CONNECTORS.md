...
配置**。v1.6.x 客户端可原样对接 v1.7.x 服务端。客户端侧唯一的新旋钮是 `DFKV_WIRE_VERSION`
（§1.3），且默认行为与 v1.6.x 逐字节一致。
...
### 1.3 wire 协议版本（v1.7.0 新增）

| env | 默认 | 说明 |
|-----|------|------|
| `DFKV_WIRE_VERSION` | `1` | `2` = 客户端 opt-in wire v2：每请求带 8 字节 `seq`，回复回显并由客户端校验（防 torn/乱序流） |

规则：
- **服务端 dual-accept v1/v2 → 无 flag-day**，客户端可独立于服务端 rollout 切换，反之亦然。
- **仅 TCP 生效。** RDMA 传输恒为 v1（零拷贝接收路径尚不适配更长的 v2 前缀），
  `DFKV_WIRE_VERSION=2` 在 RDMA 路径**静默忽略**——全局设置是安全的。
- env 每进程**读一次即缓存**，须在客户端启动前设好。
- 默认 `1` 与 v1.6.x 逐字节一致，不设永远安全。
...
| `DFKV_WIRE_VERSION` | `1` | 保持默认 | §1.3（RDMA 路径恒 v1 忽略此项） |
...
- [ARCHITECTURE.md](ARCHITECTURE.md) — 存储/协议架构（wire v2、slab、RAM 热层）
